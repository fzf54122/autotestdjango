...
class TestStrategyBackUp(TestCase):
    ...
    def test_modify_cron(self):
        ...
        response = self.session.request('PATCH', url=f'https://{host}/v2/strategy/strategy-backup-export-setting/',
                                        json=payload)
        self.assertEqual(response.status_code, 200)

    def test_backup_strategy(self):
        response = self.session.request('POST', url=f'https://{host}/v2/strategy/backups/')
