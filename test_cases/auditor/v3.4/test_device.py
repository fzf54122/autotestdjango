...
class TestDevice(TestCase):
    ...
    def list_device(self):
        ...
        response = self.session.request('GET', url=f'https://{host}/v2/device/', params=data)
        return response
    ...
    def test_create_device(self):
        ...
        response = self.session.request('POST', url=f'https://{host}/v2/device/', json=data)
    ...
    def test_delete_device(self):
        ...
        response = self.session.request('GET', url=f'https://{host}/v2/device/', params=search_data)
        rule_id = self.strategy.extract(response, '$.results.[0].id')[0]
        response = self.session.request('DELETE', url=f'https://{host}/v2/device/{rule_id}/')
        self.assertEqual(response.status_code, 204)

    def check_learning_status(self):
        response = self.session.request('GET',  url=f'https://{host}/v2/device/learned/is_learning/')
    ...
    def test_device_learning(self):
        ...
        response = self.session.request('POST', url=f'https://{host}/v2/device/study/start/', json=start_params)
        self.assertEqual(response.status_code, 200)

        while self.check_learning_status() != 0:
            time.sleep(1)
        else:
            response = self.session.request('POST', url=f'https://{host}/v2/device/learned/')
            self.assertEqual(response.status_code, 200)
    ...
    def test_enable_all(self):
        ...
        response = self.session.request('PUT', url=f'https://{host}/v2/device/', json=enable_params)
        self.assertEqual(response.status_code, 200)
    ...
    def test_disable_all(self):
        ...
        response = self.session.request('PUT', url=f'https://{host}/v2/device/', json=disable_params)
        self.assertEqual(response.status_code, 200)
